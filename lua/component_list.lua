...
register_component {
	...
	check_function = function(robot2, objpos)
		for i = 1 , objpos.x - 2 do
			if robot2:get_cell(i, objpos.y).item ~= nil then return false end
		end
		return true
	end,
	-- id accumulate better i don't know the difference
	aggregate_function = function (robot2, comp, aggregator)
		aggregator.simplepike = aggregator.simplepike or {}
		aggregator.simplepike.damage = math.max(comp.distance * (2 / 3) + 5, (aggregator.simplepike.damage or 1))
		aggregator.simplepike.number = (aggregator.simplepike.number or 1) + 1
	end,
	apply_function = function(robot2, aggregator)
...
register_component {
	...
	aggregate_function = function (robot2, comp, aggregator)
		...
		aggregator.simplelaser.damage = math.max(8 + (comp.distance / 2), (aggregator.simplelaser.damage or 1))
...
register_component {
	...
	check_function = function(robot2, objpos)
		for i = 1 , objpos.y - 1 do
			if robot2:get_cell(objpos.x, i).item ~= nil then return false end
		end
		return true
	end,
	aggregate_function = function (robot2, comp, aggregator)
...
register_component {
	...
	check_function = function(robot2, objpos)
		for i = objpos.y + 1 , robot2:size().y do
			if robot2:get_cell(objpos.x, i).item ~= nil then return false end
		end
		return true
	end,
...
register_component {
	...
	check_function = function(robot2, objpos)
		return robot2:component_name_at(objpos.x - 1, objpos.y) == "four_parted_healing_sw_w"
		and robot2:component_name_at(objpos.x, objpos.y - 1) == "four_parted_healing_ne_e"
		and robot2:component_name_at(objpos.x - 1, objpos.y - 1) == "four_parted_healing_nw_w"
	end,
	aggregate_function = function (robot2, comp, aggregator)
...
register_component {
	...
	aggregate_function = function (robot2, comp, aggregator)
		local nigbour_type = robot2:component_name_at(comp.pos.x - 1, comp.pos.y)
		if nigbour_type == "simplepike" or nigbour_type == "simplelaser" then
...
register_component {
	...
	check_function = function(robot2, objpos)
		for i = objpos.y + 2 , robot2:size().y do
			if robot2:get_cell(objpos.x, i).item ~= nil then return false end
		end
		return true
	end,
